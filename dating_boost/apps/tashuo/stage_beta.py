...
def status_tashuo_stage_beta(*, data_dir: Path) -> dict[str, Any]:
    session = _read_session(data_dir)
    report = _read_stored_json_optional(data_dir, BETA_REPORT_PATH)
...
def _read_session(data_dir: Path) -> dict[str, Any] | None:
    return _read_stored_json_optional(data_dir, BETA_SESSION_PATH)


def _read_stored_json_optional(data_dir: Path, path: Path) -> dict[str, Any] | None:
    try:
        return JsonStorage(data_dir).read_json(path, expected_schema_version=1)
...
def _read_stage_results(data_dir: Path) -> list[dict[str, Any]]:
    return JsonStorage(data_dir).read_jsonl(Path("audit") / "stage_results.jsonl")
...
def _draft_review_summary(data_dir: Path) -> dict[str, Any]:
    path = Path("audit") / "draft_reviews.jsonl"
    events = JsonStorage(data_dir).read_jsonl(path)
    if not events:
        return {"status": "not_found", "count": 0}
    return {"status": "available", "count": len(events), "path": path.as_posix()}
...
def _feedback_summary(data_dir: Path) -> dict[str, Any]:
    events = JsonStorage(data_dir).read_jsonl(BETA_FEEDBACK_PATH)
    if not events:
    ...
    return {"status": "available", "count": len(events), "by_status": counts}
