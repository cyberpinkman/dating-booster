...
def stage_tashuo_draft(
    ...
    *,
    ...
    planned_steps = _tashuo_stage_draft_steps(session)
    payload = {
    ...
    prepared = _prepare_tashuo_stage_draft_context(session, payload, output_dir=output_dir)
    if prepared.get("return_payload") is not None:
        return prepared["return_payload"]
    _execute_tashuo_stage_draft(
        session,
        payload,
        draft_text,
        planned_steps,
...
def _tashuo_stage_draft_steps(session: Any) -> list[dict[str, Any]]:
    return [
        {
...
def _prepare_tashuo_stage_draft_context(
    session: Any,
    ...
    *,
    ...
    return {"window": window, "previous_clipboard": previous_clipboard, "baseline_screen": baseline_screen}


def _execute_tashuo_stage_draft(
    session: Any,
    payload: dict[str, Any],
    draft_text: str,
    ...
    *,
    output_dir: Path | None,
    window: Any,
    previous_clipboard: dict[str, Any],
    baseline_screen: dict[str, Any],
) -> None:
    executed_steps: list[dict[str, Any]] = []
    paste_result = {"status": "not_run"}
    try:
        staged = _paste_tashuo_stage_draft(
            session,
            payload,
            draft_text,
            planned_steps,
        ...
        _verify_tashuo_stage_draft(
            ...
            window=window,
            baseline_screen=baseline_screen,
            after_screen=staged["after_screen"],
            delayed_screen=staged["delayed_screen"],
            clear_result=staged["clear_result"],
            executed_steps=executed_steps,
        )
        paste_result = staged["paste_result"]
    finally:
...
def _paste_tashuo_stage_draft(
    ...
    *,
    ...
    return {
        ...
        "delayed_screen": delayed_screen,
        "clear_result": clear_result,
        "paste_result": paste_result,
    }


def _verify_tashuo_stage_draft(
    session: Any,
    payload: dict[str, Any],
    draft_text: str,
    planned_steps: list[dict[str, Any]],
    *,
    ...
    if payload["staged_text_verification"].get("status") != "verified":
        _set_tashuo_stage_draft_ax_fallback(
            session,
            payload,
            draft_text,
            planned_steps,
...
def _set_tashuo_stage_draft_ax_fallback(
    session: Any,
    payload: dict[str, Any],
    draft_text: str,
    planned_steps: list[dict[str, Any]],
