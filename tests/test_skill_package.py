...
class SkillPackageTests(unittest.TestCase):
    def test_skill_package_metadata_is_compatible_with_capabilities(self):
        ...
        self.assertEqual(metadata["required_schema_versions"]["reply_draft"], 4)
        self.assertEqual(metadata["required_schema_versions"]["draft_review"], 1)
        self.assertEqual(metadata["required_schema_versions"]["draft_evidence"], 1)
        self.assertEqual(metadata["required_schema_versions"]["draft_generation"], 1)
        self.assertEqual(metadata["required_schema_versions"]["draft_self_review"], 1)
        self.assertEqual(metadata["required_schema_versions"]["conversation_thread"], 1)
        self.assertEqual(metadata["required_schema_versions"]["latest_turn"], 1)
        self.assertEqual(metadata["required_schema_versions"]["user_memory"], 1)
        self.assertNotIn("content_policy", metadata["required_schema_versions"])
    ...
    def test_skill_markdown_contains_required_operational_guards(self):
        ...
        self.assertIn("managed session configuration must be shown to the user", skill_text)
        self.assertIn("do not ask the user to choose max_pages_per_cycle", skill_text)
        self.assertIn("first row with no progress for 7 days", skill_text)
        self.assertIn("must present relationship_progress_report.markdown", skill_text)
        self.assertIn("do not replace it with a codex-style action summary", skill_text)
